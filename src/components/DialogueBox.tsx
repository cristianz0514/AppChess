...
export function DialogueBox({ lines, playerPortrait, playerColor, otherColor, onDone }: Props) {
  ...
  return (
    <button
      onClick={advance}
      className="relative z-10 w-full text-left rounded-2xl border p-4 transition active:scale-[0.99] backdrop-blur-sm"
      style={{ background: "oklch(0.18 0.02 60 / 0.72)", borderColor: "rgba(255,255,255,.12)" }}
    >
      {isNarration ? (
        ...
        <div className={`flex items-start gap-3 ${isPlayer ? "flex-row-reverse" : ""}`}>
          <CharacterPortrait variant={line.portrait!} bgColor={isPlayer ? playerColor : otherColor} size={40} />
